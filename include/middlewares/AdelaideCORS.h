...
struct AdelaideCORS
{
  ...
  void before_handle(crow::request &req, crow::response &res, context &ctx)
  {
      ...
      // 如果是 OPTIONS 预检请求 并且 来源被允许
      if (req.method == crow::HTTPMethod::OPTIONS && is_allowed_origin) {
          ...
          res.code = 204;
          res.end(); // 结束请求处理流程
      } else if (req.method == crow::HTTPMethod::OPTIONS && !is_allowed_origin) {
          // 如果是 OPTIONS 请求但来源不被允许，返回 403 Forbidden
          CROW_LOG_WARNING << "CORS: Forbidden OPTIONS request from origin: " << origin;
          res.code = 403;
          res.end();
      }
  ...
  void after_handle(crow::request &req, crow::response &res, context &ctx)
  {
      ...
      if (is_allowed_origin) {
          ...
          CROW_LOG_WARNING << "CORS: Actual request from disallowed origin: " << origin;
          // 注意：对于实际请求，如果来源不被允许，浏览器通常会阻止，
          // 但后端也可以选择返回 403 或不设置 CORS 头部
      } else {
           // Origin 头部为空的情况
           CROW_LOG_WARNING << "CORS: Actual request with empty Origin header from " << req.remote_ip_address;
      }
  }
};
